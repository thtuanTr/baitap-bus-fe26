...
export class DanhSachGheComponent implements OnInit {
  ...
  mangGheDangDat: any[] = [];
  @ViewChildren(ItemGheComponent) tagItemSeat: QueryList<ItemGheComponent>;

  capNhatGhe(obj) {
    if (obj.statusColor) {
      this.mangGheDangDat.push(obj.ghe);
    } 
    else{
      this.mangGheDangDat.map((item, i) => {
        if (obj.ghe.SoGhe === item.SoGhe) {
  ...
  huyGhe(soGhe, i) {
    this.mangGheDangDat.splice(i, 1);
    this.tagItemSeat.map(item => {
      if (item.ghe.SoGhe === soGhe) {
        item.statusColor = false;
      }
    })
  }
  constructor() { }
